...
def mock_dependencies():
    ...
    """
    with (
        patch("app.services.classifier._load_prompt") as mock_load_prompt,
        ...
        mock_model_instance.generate_content.return_value.text = json.dumps(
            VALID_JSON_RESPONSE
        )
...
class TestClassifyEmailUnit:
    ...
    def test_raises_error_on_non_json_response(self, mock_dependencies):
        ...
        with pytest.raises(
            InvalidResponseJsonError,
            match="A resposta da API não pôde ser decodificada como JSON",
        ):
    ...
    def test_raises_error_on_invalid_schema_response(self, mock_dependencies):
        ...
        with pytest.raises(
            InvalidClassificationResponseError, match="não contém as chaves necessárias"
        ):
            classify_email("Qualquer texto")
...
class TestValidateClassificationResponse:
    ...
    def test_passes_on_valid_data(self):
        ...
        try:
            ...
            pytest.fail(
                "Uma resposta válida não deveria lançar uma exceção de validação."
            )
    ...
    def test_fails_on_invalid_category_value(self):
        ...
        with pytest.raises(
            InvalidClassificationResponseError,
            match="O valor de 'category' .* é inválido",
        ):
            _validate_classification_response(invalid_data)
    ...
    def test_fails_on_invalid_confidence_value(self, invalid_confidence):
        ...
        with pytest.raises(
            InvalidClassificationResponseError,
            match="O valor de 'confidence' .* é inválido",
        ):
            _validate_classification_response(invalid_data)
